...
- Tightened the Inquire form spacing so the submit button appears sooner while preserving all lead fields.
...
- GA4 is controlled by `NEXT_PUBLIC_GA_MEASUREMENT_ID`; analytics queues events through `dataLayer` if the real `gtag` function has not loaded yet. Automatic GA page views are disabled and route-change page views are sent manually.
